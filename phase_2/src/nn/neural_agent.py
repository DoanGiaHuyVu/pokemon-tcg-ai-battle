...
model_path = os.path.join(os.path.dirname(__file__), "../../models/imitation_v0.pt")
...
def agent(obs: dict) -> list[int] | int:
    ...
    try:
        ...
        # 4. Action Selection
        # Mask out invalid padding options
        ...
        # Get index of max logit among valid options
        best_idx = logits[0].argmax().item()
        
        if mask[best_idx] == 0:
            best_idx = valid_indices[0].item()
            
        # print(f"[Neural Agent] Picked {best_idx} from {len(options)} options.")
        if best_idx >= len(options):
            print(f"[Neural Agent] WARNING: best_idx {best_idx} >= len(options) {len(options)}")
            return fallback_agent.agent(obs)
