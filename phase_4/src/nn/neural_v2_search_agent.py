...
def agent(obs: dict) -> list[int]:
    ...
    if state:
        ...
        n_prize = len(p1.get("prize", [0]*6))
        opp_prize = opp_deck[:n_prize]
        opp_deck = opp_deck[n_prize:]
        
        n_hand = len(p1.get("hand", [0]*7))
    ...
    # print(f"Picked {best_action} with tactical: {best_score - 0.5*policy_scores.get(tuple(best_action), 0.0):.2f}, prior: {policy_scores.get(tuple(best_action), 0.0):.2f}")
