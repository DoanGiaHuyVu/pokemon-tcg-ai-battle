...
class AgentWrapper:
    ...
    def select_action(self, obs):
        ...
        # Track multi-select
        options = obs.get("select", {}) or {}
        max_count = options.get("maxCount", 1)
        if max_count > 1:
...
def run_tournament(agent1_name, agent1_fallback, agent2_name, num_games):
    def get_agent(name, fallback):
        if name == "random":
            ...
            from src.nn.neural_v2_search_agent import agent as neural_v2_search_agent_func
            return AgentWrapper("neural_v2_search", neural_v2_search_agent_func, neural_deck, is_neural=True, use_fallback=fallback)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agent", type=str, default="neural_v1", choices=["random", "heuristic", "neural_v0", "neural_v1", "neural_v1_belief", "one_ply_search", "neural_v2", "neural_v2_search"])
