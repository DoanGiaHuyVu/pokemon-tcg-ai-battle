...
from src.nn.neural_agent_v3 import model, parser, my_deck
...
def get_neural_policy_scores(obs):
    ...
    try:
        from src.nn.neural_agent_v3 import device
        your_index = obs.get("current", {}).get("yourIndex", 0)
        # Assume Dragapult vs Random for now (deck_id=0, matchup_id=0)
        mock_step_data = {"deck_id": 0, "matchup_id": 0}
        global_features = parser._parse_global_features(obs.get("current"), step_data=mock_step_data).unsqueeze(0).to(device)
        card_tokens, numeric_features, zone_masks = parser._parse_cards(obs.get("current"), your_index)
        deck_id_tensor = torch.tensor([0], dtype=torch.long).to(device)
        
        state_inputs = {
            "deck_id": deck_id_tensor,
...
def agent(obs: dict) -> list[int] | int:
    ...
    for action in legal_actions:
        ...
        if turn_still_ours:
            ...
            beam.append((combined, action, next_ss.searchId, 1, next_ss.observation))
        else:
    ...
    for depth in range(2, MAX_DEPTH + 1):
        ...
        # Release non-surviving search IDs immediately
        for entry in beam[BEAM_WIDTH:]:
            try:
                search_release(entry[2])
            except:
                pass
        beam = []

        for parent_score, first_action, parent_sid, parent_depth, parent_obs in survivors:
            parent_options = []
            if hasattr(parent_obs, 'select') and parent_obs.select and hasattr(parent_obs.select, 'option'):
                parent_options = parent_obs.select.option
                
            num_actions = len(parent_options) if parent_options else 0
            if num_actions == 0:
            ...
            for action_idx in range(num_actions):
                ...
                if turn_still_ours and depth < MAX_DEPTH:
                    beam.append((child_score, first_action, child_ss.searchId, depth, child_ss.observation))
                else:
                    try:
                        search_release(child_ss.searchId)
                    except:
            ...
            # The parent_sid was used to generate children, it is no longer needed
            try:
                search_release(parent_sid)
            except:
                pass
